(function() {
    ...
    function updateFullscreenUI() {
        ...
        if (window.cover) {
            ...
            if (art.src !== bigCover) art.src = bigCover;
        }
    ...
    document.addEventListener('click', function(e) {
        ...
        if (link && !link.getAttribute('data-testid')?.includes('track-link')) return;
    ...
    const dismissSearch = (e) => {
        ...
        const isSearchClick = searchForm && searchForm.contains(e.target);
        
        const dropdown = e.target.closest('[data-tippy-root]') || 
                         e.target.closest('[role="listbox"]') || 
                         e.target.closest('[role="dialog"]') || 
                         e.target.closest('[role="menu"]') ||
                         e.target.closest('[data-encore-id*="popover"]') ||
                         e.target.closest('[data-encore-id*="menu"]') ||
                         e.target.closest('[data-overlayscrollbars-viewport]');
                         
        const isDropdownClick = !!dropdown;
        ...
        if (!isSearchClick) {
            if (!isDropdownClick) {
                ...
                // If inside the dropdown, only blur if clicking a play button or album art
    ...
    document.addEventListener('mousedown', dismissSearch, { capture: true });
    ...
    window.firstFuck = function() {
        ...
        pfint = setInterval(() => {
            ...
            if (window.playing && document.visibilityState == 'hidden') {
                AndBridge.wakeUp();
            } else if (!AndBridge.isWoke() && document.visibilityState == 'visible' && !hasCanvas) {
                AndBridge.wakeOff();
            }
            ...
            window.gobserver = new MutationObserver((mutations) => {
                mutations.forEach((mutation) => {
                    mutation.addedNodes.forEach((node) => {
                        if (node.nodeType === 1) {
                            // Instantly hide search dropdown / suggestions overlay to prevent split-second flash
                            const dropdownParent = node.closest('[data-tippy-root], [role="dialog"], [role="listbox"]');
                            if (dropdownParent) {
                                const hasTypeList = dropdownParent.querySelector('[data-encore-id="typeList"]') || dropdownParent.querySelector('[class*="type-list"]');
                                const hasListRowNoMenu = dropdownParent.querySelector('[data-encore-id="listRow"]') && !dropdownParent.querySelector('[role="menu"]');
                                if (hasTypeList || hasListRowNoMenu) {
                                    dropdownParent.style.setProperty('display', 'none', 'important');
                                }
                            }
    ...
    window.addAndAuto = function(once = false) {
        const run = () => {
            ...
            if (sh) {
                ...
                if (label.includes('disable smart') || checked === 'mixed') {
                    window.shmode = 'mixed';
                } else if (label.includes('disable shuffle') || label.includes('enable smart')) {
                    window.shmode = 'true';
                } else {
                    window.shmode = 'false';
                }
