        ...
        onBackPressedDispatcher.addCallback(this, object : OnBackPressedCallback(true) {
            override fun handleOnBackPressed() {
                if (AppSingleton.isLoggedIn && WebService.isServiceRunning) {
                    webView?.evaluateJavascript("(function() { if(window.closeFullscreenPlayer) { let open = !!document.getElementById('sf-fs-player'); if(open) { window.closeFullscreenPlayer(); return true; } } if(document.body.classList.contains('sf-expanded')) { let minBtn = document.querySelector('button[aria-label*=\"Minimize\"]:not(#Desktop_LeftSidebar_Id *):not(.YourLibraryX *):not([aria-label*=\"Library\"]), button[aria-label*=\"Back to player\"]'); if(minBtn) { minBtn.click(); return true; } } if(document.body.classList.contains('sf-show-nowplaying')) { if(window.triggerCloseNowPlay) { window.triggerCloseNowPlay(); } else { document.body.classList.remove('sf-show-nowplaying'); } return true; } if(document.body.classList.contains('sf-show-library')) { if(window.triggerCloseLib) { window.triggerCloseLib(); } else { document.body.classList.remove('sf-show-library'); } return true; } return false; })();") { value ->
                        if (value == "true") {
                            // Back handled by closing fullscreen player or sidebars
                        } else {
                            handleRegularBack()
                        }
                    }
